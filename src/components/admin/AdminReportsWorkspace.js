...
export const ADMIN_REPORTS = [
  {
    ...
    icon: "📊",
    description:
      "Center-wide progress, behavior, and attendance at a glance.",
  },
  {
    key: "students",
    ...
    icon: "🧒",
    description:
      "Per-child progression, goals, and domain breakdowns.",
  },
  {
    key: "teachers",
    ...
    icon: "👩‍🏫",
    description:
      "Logging compliance, checklist completion, and evaluation history by teacher.",
  },
  {
    ...
    icon: "🏫",
    description:
      "Compare classrooms on progression, attendance, and engagement.",
  },
  {
    key: "coach-performance",
    label: "Coach Performance",
    icon: "🧭",
    description:
      "Observations, follow-ups, and coaching coverage across the center.",
  },
  {
    key: "other-staff",
    label: "Other Staff Performance",
    icon: "👥",
    description:
      "Checklist and training performance for non-teaching staff.",
  },
  {
    key: "query",
    label: "Custom Query",
    icon: "🔎",
    description:
      "Build an ad-hoc report across children, classes, and date ranges.",
  },
];

export function findAdminReport(key) {
  return ADMIN_REPORTS.find((report) => report.key === key) || null;
}
...
export default function AdminReportsWorkspace({ reportKey }) {
  ...
  if (!report) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-8 text-center text-sm text-gray-600">
        ...
        <Link href="/admin/reports" className="font-semibold text-sky-700 hover:text-sky-800">
          Back to all reports
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5">
        <Link
          href="/admin/reports"
          className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-gray-400 hover:text-gray-600"
        >
          <span aria-hidden="true">&larr;</span> All reports
        </Link>
      ...
      {!centerId ? (
        <div className="rounded-2xl border border-gray-200 bg-white p-8 text-center text-sm text-gray-600">
          Select a center to view this report.
        </div>
      ) : (
        <>
          {report.key === "overview" && (
