import StaffSupplies from "@/components/staff/StaffSupplies";
...
export default function StaffChecklistWorkspace({
  Layout,
  title = "Checklist",
  staffUserId = "",
  scopeReady = true,
  ...
  const loadChecklists = useCallback(async () => {
    ...
    setError("");
    try {
      const params = new URLSearchParams({ centerId, date: selectedDate });
      // Admins hit this endpoint with their own id so the API returns the
      // personal "checklist I fill out" view instead of the management view.
      if (staffUserId) params.set("staffUserId", staffUserId);
      const data = await apiJson(`/api/v1/daily-checklists?${params.toString()}`);
  ...
  }, [centerId, selectedDate, staffUserId, scopeReady]);
      ...
      <div className="space-y-5">
        ...
        {loadingCenters ? (
          ...
          <>
            ...
            <StaffSupplies centerId={centerId} />
          </>
        )}
      </div>
    </Layout>
