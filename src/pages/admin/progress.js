      ...
      <div className="space-y-4">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          ...
          <p className="mt-1 text-sm text-gray-600">
            View overall progression across all children, filter by classroom and age group, and test individual children on goals.
          </p>
                ...
                {byChild.map((row) => {
                  ...
                  return (
                    <tr key={ch.id}>
                      ...
                      <td className="px-4 py-2 text-center text-xs font-semibold text-gray-700">{row.total}</td>
                      <td className="px-4 py-2 text-center">
                        {row.IN_PROGRESS > 0 ? <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-800">{row.IN_PROGRESS}</span> : <span className="text-gray-300">0</span>}
                      </td>
                      <td className="px-4 py-2 text-center">
                        {row.done > 0 ? <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-semibold text-emerald-800">{row.done}</span> : <span className="text-gray-300">0</span>}
                      </td>
                      <td className="px-4 py-2 text-center">
                        {row.FAILED > 0 ? <span className="rounded-full bg-red-100 px-2 py-0.5 text-xs font-semibold text-red-800">{row.FAILED}</span> : <span className="text-gray-300">0</span>}
                      </td>
                      <td className="px-4 py-2 text-center">
                        {row.NOT_STARTED > 0 ? <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-semibold text-gray-700">{row.NOT_STARTED}</span> : <span className="text-gray-300">0</span>}
                      </td>
