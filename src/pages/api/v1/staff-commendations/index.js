...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "GET") {
      ...
      const records = await prisma.staffCommendation.findMany({
    ...
    if (req.method === "POST") {
      if (session.user.role !== "ADMIN") return res.status(403).json({ error: "Only admins can add commendations" });
      ...
      const record = await prisma.staffCommendation.create({
    ...
    console.error("staff-commendations error:", e);
