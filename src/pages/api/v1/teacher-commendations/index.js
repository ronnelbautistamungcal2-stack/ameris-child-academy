...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "GET") {
      ...
      const records = await prisma.teacherCommendation.findMany({
        where: {
          centerId,
    ...
    if (req.method === "POST") {
      if (session.user.role !== "ADMIN") {
        return res.status(403).json({ error: "Only admins can add commendations" });
      }
      ...
      const record = await prisma.teacherCommendation.create({
        data: {
          centerId: bodyCenter,
    ...
    console.error("teacher-commendations error:", e);
